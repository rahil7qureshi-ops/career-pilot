...
export default function FinanceBanking() {
  ...
  const nodes = {
    summary: personal.summary ? (
      <Section title="Summary" accent="#1f2937" uppercase={false}>
        <p style={{ margin: 0, textAlign: 'justify', color: '#0f172a' }}>{personal.summary}</p>
      </Section>
    ) : null,

    experience: experience.length > 0 ? (
      <Section title="Transaction Experience" accent="#1f2937" uppercase={false}>
        {experience.map((e, i) => (
          <ExperienceRow
            key={i}
            exp={e}
    ...
    skills: skills.length > 0 ? (
      <Section title="Technical Proficiencies" accent="#1f2937" uppercase={false}>
        <div style={{ color: '#0f172a', lineHeight: 1.7 }}>{skills.map((s) => s.name).join(' · ')}</div>
      </Section>
    ) : null,

    certifications: certifications.length > 0 ? (
      <Section title="Additional Certifications" accent="#1f2937" uppercase={false}>
  ...
  return (
    ...
    >
      ...
      <OrderedSections
        ...
        customBodyStyle={{ color: '#0f172a' }}
        header={
          <Section title="Licenses & Designations" accent="#1f2937" uppercase={false}>
