...
export default function ConsultingCase() {
  ...
  const nodes = {
    summary: personal.summary ? (
      <Section title="Executive Summary" accent="#1f2937" variant="plain" uppercase={false}>
        <p style={{ margin: 0, textAlign: 'justify' }}>{personal.summary}</p>
      </Section>
    ) : null,

    // Cases is exposed as a header slot since the consultant's value is the
    // case-study layout itself, not the underlying experience order.
    experience: cases.length > 0 ? (
      <Section title="Case Studies" accent="#1f2937" uppercase={false}>
        {cases.map((c, i) => (
    ...
    skills: skills.length > 0 ? (
      <Section title="Capabilities" accent="#1f2937" uppercase={false}>
